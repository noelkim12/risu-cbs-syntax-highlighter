/**
 * CBS Core - Database and Parser
 * Standalone JavaScript version (no dependencies)
 */

// CBS Function Database
const CBSDatabase = (function () {
  const functions = {};

  // Helper to add function with aliases
  function addFunction(info) {
    functions[info.name.toLowerCase()] = info;

    // Add all aliases
    for (const alias of info.aliases) {
      functions[alias.toLowerCase()] = info;
    }
  }
  ...
  addFunction({
    ...
    arguments: ["a", "b"],
    example:
      "{{and::1::1}} 또는 {{and::{{equal::{{user}}::Alice}}::{{greater::{{getvar::score}}::10}}}}",
  });
  ...
  addFunction({
    ...
    arguments: ["a", "b"],
    example:
      "{{or::1::0}} 또는 {{or::{{equal::{{char}}::Bob}}::{{equal::{{char}}::Alice}}}}",
  });
  ...
  addFunction({
    ...
    arguments: ["array", "..."],
    example:
      "{{all::1::1::1}} 또는 {{all::{{equal::{{user}}::Alice}}::{{greater::{{getvar::level}}::5}}}}",
  });
  ...
  addFunction({
    ...
    arguments: ["array", "..."],
    example:
      "{{any::0::1::0}} 또는 {{any::{{equal::{{user}}::Alice}}::{{equal::{{user}}::Bob}}}}",
  });
  ...
  addFunction({
    ...
    example:
      "{{replace::hello::l::r}} 또는 {{replace::{{user}}'s sword::sword::shield}}",
  });
  ...
  addFunction({
    ...
    example:
      "{{arraysplice::[a,b,c]::1::1::x}} 또는 {{arraysplice::[1,2,3,4]::2::2}}",
  });
  ...
  addFunction({
    ...
    example:
      "{{makearray::a::b::c}} 또는 {{makearray::{{user}}::{{char}}::{{getvar::npc}}}}",
  });

  addFunction({
    name: "filter",
    description:
      "배열을 필터링합니다 (unique: 중복 제거, nonempty: 빈 값 제거, all: 모두)",
    aliases: [],
    arguments: ["array", "type"],
    example:
      "{{filter::[a,,b,a]::unique}} 또는 {{filter::[a,,b,,c]::nonempty}} 또는 {{filter::[x,,y,x,z]::all}}",
  });
  ...
  addFunction({
    ...
    example:
      "{{calc::2+2}} 또는 {{calc::{{getvar::hp}} - 10}} 또는 {{calc::(5 + 3) * 2}}",
  });
  ...
  addFunction({
    ...
    example:
      "{{date::YYYY-MM-DD::1620000000000}} 또는 {{date::HH:mm:ss::{{unixtime}}}} 또는 {{date::MMMM Do YYYY, h:mm a::{{unixtime}}}}",
  });
  ...
  addFunction({
    name: "inlayeddata",
    description:
      "스타일과 데이터가 포함된 인레이 이미지를 반환합니다 (AI에 전송됨)",
    aliases: [],
    arguments: ["name"],
  ...
  addFunction({
    name: "pick",
    description:
      "random과 같지만 메시지 인덱스로 시드가 고정되어 일관된 출력을 제공합니다",
    aliases: [],
    arguments: ["arg1", "arg2", "arg3", "..."],
    example: "{{pick::a::b::c}}",
  });
  ...
  addFunction({
    name: "rollp",
    description:
      "roll과 같지만 메시지 인덱스로 시드가 고정되어 일관된 출력을 제공합니다",
    aliases: [],
    arguments: ["number"],
    example: "{{rollp::6}}",
  });
  ...
  addFunction({
    name: "metadata",
    description:
      "메타데이터 값을 가져옵니다 (mobile, local, node, version, lang 등)",
    aliases: [],
    arguments: ["key"],
  ...
  addFunction({
    name: "slot",
    description:
      "반복 중인 현재 요소를 반환합니다. 인자 없이 사용하면 컨텍스트에 따라 다른 값을 반환하고, 이름을 지정하면 해당 이름의 슬롯 값을 반환합니다",
    aliases: [],
    arguments: ["name?"],
  ...
  addFunction({
    name: "xor",
    description:
      "XOR 암호를 사용하여 문자열을 암호화하고 base64로 인코딩합니다 (0xFF 키 사용)",
  ...
  addFunction({
    name: "xordecrypt",
    description:
      "base64로 인코딩된 XOR 암호화 문자열을 원본 텍스트로 복호화합니다",
    aliases: ["xordecode", "xord"],
    arguments: ["base64string"],
    example:
      "{{xordecrypt::aGVsbG8=}} 또는 {{xordecrypt::{{getvar::encrypted}}}}",
  });

  addFunction({
    name: "crypt",
    description:
      "사용자 정의 시프트 값으로 카이사르 암호 암호화/복호화를 적용합니다 (기본값: 32768). 기본 시프트를 사용하면 암호화와 복호화 모두에 사용할 수 있습니다",
    aliases: ["crypto", "caesar", "encrypt", "decrypt"],
    arguments: ["string", "shift?"],
    example:
      "{{crypt::hello}} 또는 {{crypt::hello::1000}} 또는 {{crypt::{{crypt::message}}}} (암호화 후 복호화)",
  });
  ...
  addFunction({
    name: "comment",
    description:
      "코드 주석을 위한 CBS 함수입니다. 채팅에 표시되지만 처리되지 않습니다",
    aliases: [],
    arguments: ["text"],
    example:
      "{{comment::이것은 주석입니다}} 또는 {{comment::TODO: 나중에 수정}}",
  });
  ...
  addFunction({
    ...
    description:
      "여는 괄호 (를 표시하지만 파싱에 영향을 주지 않는 특수 유니코드 문자를 반환합니다",
    aliases: ["debo", "("],
    arguments: [],
  ...
  addFunction({
    ...
    description:
      "닫는 괄호 )를 표시하지만 파싱에 영향을 주지 않는 특수 유니코드 문자를 반환합니다",
    aliases: ["debc", ")"],
    arguments: [],
  ...
  addFunction({
    ...
    description:
      "여는 꺾쇠괄호 <를 표시하지만 HTML 파싱에 영향을 주지 않는 특수 유니코드 문자를 반환합니다",
    aliases: ["deabo", "<"],
    arguments: [],
  ...
  addFunction({
    ...
    description:
      "닫는 꺾쇠괄호 >를 표시하지만 HTML 파싱에 영향을 주지 않는 특수 유니코드 문자를 반환합니다",
    aliases: ["deabc", ">"],
    arguments: [],
  ...
  addFunction({
    ...
    description:
      "콜론 :을 표시하지만 CBS 인자 구분자로 파싱되지 않는 특수 유니코드 문자를 반환합니다",
    aliases: ["dec", ":"],
    arguments: [],
  ...
  addFunction({
    ...
    description:
      "세미콜론 ;을 표시하지만 파싱에 영향을 주지 않는 특수 유니코드 문자를 반환합니다",
    aliases: [";"],
    arguments: [],
  ...
  addFunction({
    ...
    description:
      "미리 빌드된 에셋 제외 설정으로 필터링된 캐릭터 표시 에셋 이름의 JSON 배열을 반환합니다",
    aliases: [],
    arguments: [],
  ...
  addFunction({
    name: "source",
    description:
      '사용자 또는 캐릭터의 프로필 이미지 소스 URL을 반환합니다. 인자는 "user" 또는 "char"이어야 합니다',
    aliases: [],
    arguments: ["type"],
  ...
  addFunction({
    name: "position",
    description:
      "@@position <positionName> 데코레이터와 같은 다양한 기능에서 사용할 수 있는 위치를 정의합니다",
    aliases: [],
  ...
  addFunction({
    name: "#when",
    description:
      'CBS의 조건문입니다. 1과 "true"는 참이고, 그 외는 거짓입니다. 다양한 연산자를 지원합니다 (and, or, is, isnot, >, <, >=, <=, not, keep, legacy, var, vis, vnotis, toggle, tis, tnotis)',
    aliases: [],
    arguments: ["condition", "operator?", "..."],
    example:
      "{{#when condition}}...{{/when}} 또는 {{#when::A::and::B}}...{{/when}} 또는 {{#when::keep::not::condition}}...{{/when}}",
  });

  addFunction({
    name: ":else",
    description:
      "CBS의 else 문입니다. {{#when}} 내부에서 사용되어야 합니다. {{#when}}이 여러 줄인 경우 :else는 추가 문자열 없이 별도 줄에 있어야 합니다",
    aliases: [],
    arguments: [],
  ...
  addFunction({
    name: "#puredisplay",
    description:
      "CBS 처리 없이 내용을 표시합니다. 원시 HTML이나 다른 콘텐츠를 파싱 없이 표시할 때 유용합니다",
    aliases: [],
    arguments: [],
  ...
  addFunction({
    name: "#each",
    description:
      '배열이나 객체를 반복합니다. 객체의 경우 "as key" 구문으로 키에 접근할 수 있습니다',
    aliases: [":each"],
    arguments: ["array_or_object", "as?", "key?"],
    example:
      "{{#each array}}...{{/each}} 또는 {{#each object as key}}{{slot::key}}...{{/each}}",
  });

  addFunction({
    name: "//",
    description:
      "코드를 주석 처리하는 CBS 주석입니다. 출력에 표시되지 않습니다",
    aliases: [],
    arguments: ["comment"],
    example: "{{// 이것은 주석입니다}}",
  });
  ...
  addFunction({
    name: "hiddenkey",
    description:
      "로어북 활성화를 위한 키로 작동하지만, 모델 요청에는 포함되지 않습니다",
    aliases: [],
    arguments: ["value"],
  ...
  addFunction({
    name: "trigger_id",
    description:
      '수동 트리거를 발생시킨 클릭된 요소의 risu-id 속성에서 ID 값을 반환합니다. ID가 제공되지 않은 경우 "null"을 반환합니다',
    aliases: ["triggerid"],
    arguments: [],
    example: "{{trigger_id}}",
  });
  ...
  addFunction({
    name: "jb",
    description:
      "AI 동작을 수정하는 데 사용되는 탈옥(jailbreak) 프롬프트 텍스트를 반환합니다. 텍스트는 변수 치환을 위해 채팅 파서를 통해 처리됩니다",
    aliases: ["jailbreak"],
    arguments: [],
    example: "{{jb}}",
  });
  ...
  addFunction({
    name: "jbtoggled",
    description:
      '탈옥 프롬프트가 현재 활성화/켜져 있으면 "1"을 반환하고, 비활성화되어 있으면 "0"을 반환합니다. 전역 탈옥 토글 상태를 반영합니다',
    aliases: [],
    arguments: [],
    example: "{{jbtoggled}}",
  });
  ...
  return {
    ...
    // For extending with more functions
    addFunction: addFunction,
  };
})();

// CBS Parser
const CBSParser = (function () {
  /**
   * Find CBS context at cursor position
   * @param {string} text - Full text
   * @param {number} cursorPos - Cursor position (character offset)
   * @returns {object|null} Context information
   */
  function findCBSContext(text, cursorPos) {
    ...
    // Check if we're still inside CBS
    const afterOpen = text.substring(lastOpenIndex, cursorPos);
    ...
    if (
      separatorIndex !== -1 &&
      (spaceIndex === -1 || separatorIndex < spaceIndex)
    ) {
    ...
    context.functionName = cbsContent
  ...
  /**
   * Find all bracket pairs with nesting levels
   * @param {string} text - Full text
   * @returns {Array} Array of bracket pair objects
   */
   ...
   * @returns {object|null} Function call context
   */
  function findFunctionCall(text, cursorPos) {
    const beforeCursor = text.substring(0, cursorPos);
    const lastOpenIndex = beforeCursor.lastIndexOf("{{");
    ...
    let cbsContent = afterOpen.substring(2).replace(/^[#/:]+/, "");

    const firstSeparator = cbsContent.indexOf("::");
    if (
      firstSeparator === -1 ||
      cursorPos <= lastOpenIndex + 2 + firstSeparator
    ) {
      return null;
    }

    const functionName = cbsContent.substring(0, firstSeparator).trim();
    const argumentText = cbsContent.substring(firstSeparator + 2);

    // Count arguments (by counting ::)
    let argumentCount = 0;
    let depth = 0;
  ...
  // Public API
  return {
    findCBSContext: findCBSContext,
    findBracketPairs: findBracketPairs,
...
if (typeof window !== "undefined") {
  window.CBSDatabase = CBSDatabase;
  window.CBSParser = CBSParser;
}
